...
{
    [Route("[controller]")]
    public class ModuloController : ApiBase
    {
        private readonly IModuloService _service;
        private readonly IValidator<Modulo> _validator;

        public ModuloController(NotificationContext notificationContext, IModuloService service, IValidator<Modulo> validator)
        {
        ...
        public JsonReturn Post([FromBody]Modulo modulo)
        {
            if (modulo == null)
                return ReturnJson("Por favor, passe alguma informação.", (int)HttpStatusCode.BadRequest);

            ValidationResult results = _validator.Validate(modulo, options => options.IncludeRuleSets("insert"));
        ...
        [HttpPut]
        [Authorize(Roles = "manager")]
        public JsonReturn Put([FromBody]Modulo modulo)
        {
            if (modulo == null)
                return ReturnJson("Por favor, passe alguma informação.", (int)HttpStatusCode.BadRequest);

            ValidationResult results = _validator.Validate(modulo, options => options.IncludeRuleSets("update"));
